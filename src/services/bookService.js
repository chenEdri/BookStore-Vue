...
function getEmptyForReview() {
  return { name: '', rate: '', date: '', text: '' };
}
...
function addReview(bookId, review) {
  ...
  let bookToEdit = gbooks.filter(book => book.id === bookId);
  bookToEdit = { ...book, review: review }
  let books = gbooks.map(book => book.id === bookToEdit.id ? bookToEdit : book)
  return books;
}

function remove(petId) {
  books = gbooks.filter(book => book.id !== bookId)
}
...
function _add(book) {
  const bookFromStorage = Storage.loadFromStorage('books');
  const bookToAdd = {
    ...book,
    id: makeId()
  }
  gbooks = [bookToAdd, ...bookFromStorage]
  Storage.saveToStorage('books', books)
  window.thebooks = books
}

function _update(bookToSave) {
  gbooks = gbooks.map(book => book.id === bookToSave.id ? bookToSave : book)
  return bookToSave
}

// function getbookListFromApi(text) {
//   var newBooks;
//   return _callbookSearchApi(text)
//     .then(books => {
//       console.log(books);
...
//             description: (book.volumeInfo.description) ? book.volumeInfo.description : 'not found',
//             pageCount: book.volumeInfo.pageCount,
//             categories: (book.volumeInfo.categories) ? book.volumeInfo.categories : 'not found',
//             thumbnail: (book.volumeInfo.imageLinks && book.volumeInfo.imageLinks.thumbnail) ? book.volumeInfo.imageLinks.thumbnail : 'not dound',
//             language: book.volumeInfo.language,
//             listPrice: (book.saleInfo.listPrice) ? ({
//               amount: (book.saleInfo.listPrice.amount) ? book.saleInfo.listPrice : 'not fount',
//               currencyCode: (book.saleInfo.currencyCode) ? book.saleInfo.currencyCode : '$',
//               isOnSale: (book.saleInfo.isOnSale) ? book.saleInfo.isOnSale : false
//             }) : ({
//               amount: 100,
//               currencyCode: '$',
//               isOnSale: false
...
// function _callbookSearchApi(text) {//change to text in the wright place
//   return axios.get(`https://www.googleapis.com/books/v1/volumes?q=${text}&key=AIzaSyDmya2Az9xeHL5r5HgyOL29s2hRTyUixLM`)
//     .then(res => res.data)
// }
...
function getNextPrev(bookId) {
  ...
  // const nextPetId = pets[bookIdx + 1]?  pets[bookIdx + 1]: pets[0]
  // The SAME!!
  const nextBook = gbooks[bookIdx + 1] || books[0]
  const prevBook = gbooks[bookIdx - 1] || books[books.length - 1]
  return {
    prevBooktId: prevBook.id,
    nextBookId: nextBook.id
  }
}
function loadFromStorage(key) {
  return Storage.loadFromStorage(key);
}


function _loadBooks() {
  var books;
  const booksFromStorage = Storage.loadFromStorage('books');
  (booksFromStorage && booksFromStorage.length !== 0) ? books = booksFromStorage : books = gbooks;
    Storage.saveToStorage('books', books);
    return books;
}
